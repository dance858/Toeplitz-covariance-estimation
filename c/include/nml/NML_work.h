...
typedef struct
{
    ...
    int verbose;
    double
        tau; /* factor to increase diagonal entries of Hessian if Cholesky fails */
    int max_iter;
    ...
    // double *hess;                   /* hess = [hess_xx, hess_yx^H; hess_yx,
    // hess_yy], size (2n+1) x (2n+1) */
    double *hess_packed;      /* packed storage (lower triangular part, column-major
                                 order) of hess, size (2n+1)*(2n+2)/2 */
    double *chol_hess_packed; /* packed storage (lower triangular part, column-major
                                 order) of lower Cholesky factor of Hessian */
    double *hess_evals;       /* eigenvalues of Hessian, array of size (2n + 1) */

    double complex *chol_toep;      /* packed storage (column-major order) of lower
                                       Cholesky factor of T(x, y)^{-1} */
    double complex *full_chol_toep; /* full storage (column-major order) of lower
                                       Cholesky factor of T(x, y)^{-1} */
    double *sigma2;                 /* required for the Levinson-Durbin algorithm */
    double complex *L_full; /* full storage (column-major order) of lower Cholesky
                               factor of sample covariance */
...
typedef struct
{
    ...
    int diag_init_succeded;     /* 1 if the diagonal initialization yielded a valid
                                   initial point, otherwise zero */
