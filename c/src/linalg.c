...
void zaxpy_variant(const int n, const double complex *a, double complex *x,
                   double complex *y)
{
    for (int i = 0; i < n; i++)
    {
...
void pad_with_zeros(double complex *A, double complex *A_padded, int nrows,
                    int ncols, int N)
{
    int col, ii;
    memset(A_padded, 0,
           N * ncols * sizeof(double complex)); /* Set A_padded to zero */
    for (col = 0; col < ncols; col++)
    {
        for (ii = 0; ii < nrows; ii++) A_padded[col * N + ii] = A[col * nrows + ii];
    }
}
