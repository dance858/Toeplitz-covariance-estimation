"""DOA estimation MSE vs number of snapshots."""
import numpy as np
...
def main():
    ...
    samples = np.arange(15, 501, 20)
    MC_runs = 1000

    # Containers
    MSE_SC = np.zeros(len(samples))
    MSE_NML = np.zeros(len(samples))
    MSE_AML = np.zeros(len(samples))
    MSE_DA = np.zeros(len(samples))
    ...
    for ii, K in enumerate(samples):
        ...
        for run in range(MC_runs):
            Y, true_cov = generate_ula_data(power_source, sig2, d, m, K,
    ...
    # Plot
    plt.figure()
    plt.semilogy(samples, MSE_SC, "-o", label="SC")
    plt.semilogy(samples, MSE_NML, "-s", label="NML")
    plt.semilogy(samples, MSE_AML, "-^", label="AML")
    plt.semilogy(samples, MSE_DA, "-v", label="DA")
    plt.semilogy(samples, crb_sto, "--", label="U-CRB")
    plt.semilogy(samples, crb_sto_uc, "--", label="S-CRB")
    plt.xlabel("N")
    plt.ylabel("MSE")
    plt.legend()
    plt.grid(True)
    plt.savefig(os.path.join(os.path.dirname(__file__), "mse_vs_samples.png"),
                dpi=150, bbox_inches="tight")
    print("Saved mse_vs_samples.png")
